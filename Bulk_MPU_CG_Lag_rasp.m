...
[initLag,startClocks,endClocks, delays] = ProcessGoodClockLog('Leon_T2022_06_02___15_17_26_002891_Log');
...
%             plot(round(-delays(index)):1:length(angles_deg_f(:,finger))+round(-delays(index))-1,-(angles_deg_f(:,finger)+offset)./(angles_deg_f(CGm,finger)+offset))
%             plot(-(angles_deg(:,finger)+offset)./(angles_deg(CGm,finger)+offset))
%             hold on
%             plot((-accAngle(start:start+length(angles_deg_f(:,finger))-1))./accAngle(start+MPUm))
%             hold on
            plot(diff(-(angles_deg_f(:,finger)+offset)./(angles_deg_f(CGm,finger)+offset)))
            hold on
            plot(diff(MPUf))
            hold on
            legend("CyberGlove","MPU"); 
            title(index)
...
aSEM = std((lengthLag/2)-lags)/sqrt(length((lengthLag/2)-lags));               % Standard Error
ats = tinv([0.025  0.975],length((lengthLag/2)-lags)-1);      % T-Score
aCI = mean((lengthLag/2)-lags) + ats*aSEM;    
