function [f] = TekScanGraph_F(task_filename,stimOn, stimOff)
...
f = figure('units','inch','position',[5, 4, 10, 4]);
for i=1:1; subplot(1,2,i);
Im=pcolor(data_f(:,:,MaxT));cb=colorbar;colormap jet;caxis([0,max(max(data_f(:,:,MaxT)))]);
...
for i=2:2; subplot(1,2,(i));
plot(frame(st:et)/200,sumF(st:et),'k','LineWidth',2);box off; ax=gca;ax.LineWidth=2;ax.TickDir='Out';hold on;
xline((MaxT/200),'color','b');hold on;xline((stimOff),'color','r','LineStyle',':','LineWidth',3);hold on;xline((stimOn),'color','r','LineStyle',':','LineWidth',3);hold off;ylabel('Force (N)');xlabel('Time (s)');
set(gca,'FontSize',16);
