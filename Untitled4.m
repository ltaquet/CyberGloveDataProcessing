...
for i = 1:1:length(unprocessed_files)
    ...
    breakpts = find(B);
    mov_index = 1;
    for j = 2:1:length(breakpts)-1
        angles       = anglesW(breakpts(j):breakpts(j+1), :);
        angles_deg   = angles_degW(breakpts(j):breakpts(j+1), :);
        angles_deg_f = angles_deg_fW(breakpts(j):breakpts(j+1), :);
        angles_f     = angles_fW(breakpts(j):breakpts(j+1), :);
        data_time    = data_timeW(breakpts(j):breakpts(j+1), :);
        rawData      = rawDataW(breakpts(j):breakpts(j+1), :);
